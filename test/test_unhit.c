...
int main(int argc, char *argv[])
{
	...
	err = get_next_unhit_func(namebuf, 128);
	printf("err: %d, namebuf: %s\n", err, namebuf);
	memset(namebuf, 0, 128);
	err = get_next_unhit_cp(namebuf, 128);
	printf("err: %d, namebuf: %s\n", err, namebuf);

	size_t map_len = 0;
	char *map_buff = NULL;
	err = get_path_map(map_buff, &map_len);
	while (errno == EAGAIN) {
		printf("do it again, new map_len: %lx\n", map_len);
		map_buff = malloc(map_len);
		if (map_buff)
			goto out;
		memset(map_buff, 0, map_len);
		sleep(1);
		err = get_path_map(map_buff, &map_len);
	}
	write(2, map_buff, map_len);
